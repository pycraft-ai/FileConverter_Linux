...
### 隐私保护
- **文件加密落盘**：用户上传/转换输出的文件在服务器磁盘上以 **Fernet 对称加密**存储，即使服务器被入侵也无法直接读取用户内容
- **转换明文零残留**：转换前临时解密到内存，转换后立即删除临时明文
- **文件自动彻底删除**：上传与输出文件默认保留 **24 小时** 后自动清理（`FILE_CLEANUP_TTL` 可调）
- **目录权限收紧**：`uploads/`、`outputs/` 及文件权限收紧为 `700`/`600`，防止同机其他用户读取
- **隐私政策页面**：向用户明确说明文件处理流程、保留期限、存储方式与使用建议
...
2. **文件清理** — `uploads/` 和 `outputs/` 中的超时文件（默认 24 小时，由 `FILE_CLEANUP_TTL` 控制）由后台任务每小时自动清理，应用退出时也会清理
